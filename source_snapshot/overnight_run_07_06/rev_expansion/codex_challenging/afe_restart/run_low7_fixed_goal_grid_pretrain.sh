...
if [[ "$PHASE" == "data" ]]; then
  [[ ! -e "$RUN_ROOT" ]] || { echo "data output root already exists: $RUN_ROOT" >&2; exit 1; }
  mkdir -p "$RUN_ROOT/logs" "$RUN_ROOT/shards" "$RUN_ROOT/viz"
  "$PYTHON" -m "$MODULE" endpoints --fixed-goal-grid --seed 0 --output "$ENDPOINTS" \
    >"$RUN_ROOT/logs/endpoints.log" 2>&1
  "$PYTHON" -m "$MODULE" starts --endpoint-manifest "$ENDPOINTS" \
    --output "$RUN_ROOT/viz/fixed_goal_full_grid_starts.png" \
    >"$RUN_ROOT/logs/starts.log" 2>&1
  ...
  for gamma in "${GAMMAS[@]}"; do
    ...
    "$PYTHON" -m "$MODULE" collect \
      ...
      --gamma "$gamma" \
      --outdir "$RUN_ROOT/shards/gamma_$tag" \
      --device cuda:0 \
  ...
  "$PYTHON" -m "$MODULE" combine \
    --shard-manifests "${manifests[@]}" --outdir "$COMBINED" \
    >"$RUN_ROOT/logs/combine.log" 2>&1
  "$PYTHON" -m "$MODULE" render --manifest "$COMBINED/manifest.json" \
    >"$RUN_ROOT/logs/render.log" 2>&1
  "$PYTHON" -m "$MODULE" video --manifest "$COMBINED/manifest.json" \
...
EVALUATION="$RUN_ROOT/pretrained_eval_m20_nfe12"
...
"$PYTHON" -m afe_restart.evaluate_low7_pretrained \
  ...
  --device cuda:0 \
  --verifier-workers 16 \
  >"$RUN_ROOT/logs/pretrained_eval_m20_nfe12.log" 2>&1
