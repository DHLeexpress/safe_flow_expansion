...
| **B1 max-margin metric goal — calibrated sampler** | Switched expansion execution to nominal-Hp-gated single-step maximum margin; used cap 768 / ESS 0.5 / signed NVP $\alpha=0.01$ and selected r15. A per-$\gamma$ initial-noise temperature map was calibrated separately and frozen before a new disjoint M50 confirmation. | All 350 episodes succeeded: SR 100%, CR 0%, $V_{\rm safe}=96$--100%; clearance decreases overall from 0.0683 m at $\gamma=0.1$ to 0.0501 m at $\gamma=1$, while time falls from 18.86 s to 14.50 s. | This is a calibrated sampling-policy result, not raw temperature-1 intrinsic-generator performance. Temperature selection is an explicit controller knob and must not be presented as AFE training alone. |
