...
| **Execution-rule rows + adaptive gamma at M=200 (2026-07-21)** | Re-evaluated every cost-arm round checkpoint at M=200/gamma; trained two execution-rule variants with the otherwise-identical recipe (SOCP-gated max-step-progress; ungated horizon-progress baseline at M=100); swept and back-traced an adaptive-gamma scheduler (behavioral-margin gradient, alpha=1, beta=0.1); applied a declared per-gamma evaluation-temperature schedule for gamma 0.1/0.2. Study: [`EXECUTION_ROWS_STUDY.md`](EXECUTION_ROWS_STUDY.md), figures `assets/paper/b1_exec_rows.*`. | Gated max-progress execution is ~0.3-0.6 s faster at a modest V_safe cost (~.83 vs ~.95 pooled r20); the ungated baseline proves the nominal-Hp/SOCP gate, not progress ranking, carries safety (V_safe plateau .83). Adaptive gamma beats every fixed pooled round on time (15.4 vs 16.7 s) at matched safety (CR .01, V_safe .98 at r19). Low-temperature evaluation of gamma 0.1 reads out mode behavior: V_safe(0.1) .40 -> 1.00 with clearance highest of all gammas — the temp-1 jitter, not the mode, fails the SOCP certificate. | The temperature schedule is an evaluation-time presentation choice, fully recorded per cell; temp-1 series are retained unmodified; gamma=0.2 late V_safe dips to ~.91 from the ordering-preserving entries; adaptive selection used finite m=100 screening at r19. |
