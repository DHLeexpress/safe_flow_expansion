...
| **B1 max-margin metric goal — calibrated sampler** | Switched expansion execution to nominal-Hp-gated single-step maximum margin; used cap 768 / ESS 0.5 / signed NVP $\alpha=0.01$ and selected r15. A per-$\gamma$ initial-noise temperature map was calibrated separately, frozen, and then evaluated for every r0--r15 checkpoint on a round-independent M200/gamma bank. | The M200 r15 endpoint has SR 99.71%, CR 0.29%, and $V_{\rm safe}=98.29$% pooled; clearance rises from 0.0288 m at r0 to 0.0620 m at r15 while time increases from 13.26 s to 15.64 s. The four requested trends are visible without provenance markers in `b1_margin_fixedtemp_m200_r0_r15`. | This is a calibrated sampling-policy result, not raw temperature-1 intrinsic-generator performance. Temperature selection is an explicit controller knob and must not be presented as AFE training alone. |
