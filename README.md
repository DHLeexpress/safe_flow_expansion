...
| **B1 paper figures — lane-resolved M50 re-rendering (2026-07-20)** | Added `scripts/paper_b1_evolution_curves.py` and `scripts/paper_b1_mode_gallery.py` plus the per-round M10 screening copy `provenance/b1_current_best/screening_m10_metrics.jsonl`; no new rollouts and no new science — the mode gallery re-renders all 50 retained raw temperature-1 M50 confirmation trajectories per (round, $\gamma$) cell. | The U/R binary undercounts what B1 covers: classifying successes by the signed closest-approach lane offset shows **four** separated lanes (inner/outer per side). At r19 all four lanes are populated at every rendered $\gamma$ with sharp offset clusters (e.g. $\gamma{=}0.5$: RO 9 / RI 16 / UI 17 / UO 7, 1 fail), while r0 leaves the $\gamma{=}0.5$ outer lanes nearly empty (RO 2 / UO 3, 30 fails) with smeared offsets. | The inner/outer split is a fixed $|{\rm offset}|=0.7$ heuristic on the closest-approach offset, not a homotopy classification; the per-round curves are M10 screening evidence and only the overlaid M50 stars are confirmation. |
| **Kazuki collapse test + faithful transplant (2026-07-21)** | Ran the ported CFM-MPPI baseline at M=50 in three windowed arms (guidance-zero, all-zero, their full mixed-coef protocol), plus a full-horizon variant with chained-window proposals and their refinement/dilution, and a workspace-containment repair arm; all against the raw M50 cell `r000_g0.5`. Full audit: [`KAZUKI_FAITHFUL.md`](KAZUKI_FAITHFUL.md). | `goal_coef=safe_coef=0` does **not** collapse to raw pretrained (raw SR 0.40 vs 100% start-corner timeout); zeroing the MPPI stage cost too approximately does (SR 0.44, U/R 10/12). Faithfully applied, the method never engages the giant obstacle: windowed = corner freeze (TO 1.0), full-horizon = arena-exit cost runaway (CR 0.96; their open-space cost's global minimum on a walled scene is to leave it), repaired = perimeter orbit (SR 0.40 = proposer's raw SR, CR 0.13, TO 0.47). Its own t=0 proposals contain the routes (133/200 near-goal, U/R 106/94); the refinement layer discards them. | The per-step full-sequence flow re-projection — the component a windowed FM cannot provide — is load-bearing in their native method; cost weights were transplanted without re-tuning; bounds arm is M=15. |
| **Native-cost Kazuki gallery diagnostic (2026-07-21)** | Kept Kazuki-style safety guidance but replaced generated-mode ranking, MPPI perturbation weighting, and refined-mode selection with one frozen scorer: the exact B1 SafeMPPI plan cost. Fixed `goal_coef=0`, conditioned on the displayed $\gamma$, swept `safe_coef`, and selected the two non-perfect settings whose outcomes—not merely route shapes—changed across $\gamma$. | The displayed rows are `safe_coef=0.7` and `0.9`. In M10/$\gamma$, only $\gamma=0.1$ succeeds (10% and 20%); $\gamma=0.5,1.0$ remain at 0% SR. This exposes a visible gamma effect without presenting a perfect baseline. | This is a diagnostic hybrid, not a faithful reproduction of the external method: its guidance remains Kazuki-style, its refinement cost is native B1, and M10 is too small for a performance claim. |
...
| **B1 max-margin metric goal — calibrated sampler** | Switched expansion execution to nominal-Hp-gated single-step maximum margin; used cap 768 / ESS 0.5 / signed NVP $\alpha=0.01$ and selected r15. The revised map uses temperature 0.5 at $\gamma=0.1$ and temperature 1 at $\gamma=0.7,1.0$; unchanged cells are reused from the same round-independent M200/gamma bank. | The revised M200 r15 endpoint has SR 98.93%, CR 1.07%, and $V_{\rm safe}=94.50$% pooled. Clearance rises from 0.0243 m at r0 to 0.0602 m at r15, but the late $\gamma=0.1$ clearance decline and 5% r15 collision rate at $\gamma=1$ remain visible under gamma-only 95% bands. | This is a calibrated sampling-policy result, not raw temperature-1 intrinsic-generator performance. The revised curve deliberately exposes residual instability rather than hiding it with lower evaluation temperatures. |
...
![B1 revised fixed-temperature M200 trends with 95% bands](assets/paper/b1_margin_fixedtemp_m200_revised_r0_r15.png)
